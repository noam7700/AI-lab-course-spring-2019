...
KnapsackGene::KnapsackGene(double max_weight, bool isUsingNiching, float sigma_share):
    Gene(0, 1.0f, 0.0f, isUsingNiching, sigma_share), knapsack(vector<bool>()), max_weight(max_weight), weight(0), value(0) {}

bool KnapsackGene::operator<(Gene& rh) {
	return this->fitness < rh.getFitness();
}

float KnapsackGene::dist(Gene& rh){
    //TODO: the bigger the difference return bigger
    return 1.0f;
}
